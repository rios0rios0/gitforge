---
name: code-review
description: "Review pull requests and diffs in gitforge — the shared Go library abstracting GitHub, GitLab, Azure DevOps and Codeberg — against the rios0rios0/guide standards, with extra weight on public-API stability, the bounded-context layout, commit signing, and token handling. Use when reviewing a PR, a branch, or staged changes here."
---

# Code review — `gitforge`

`gitforge` is a **library** consumed by `autobump`, `autoupdate`, `code-guru`, and `dev-toolkit`. It provides provider discovery, pull-request management, API file operations, go-git local operations, GPG/SSH commit signing, Keep a Changelog processing, token resolution, and factory-based registries. A defect here reaches every consumer at once.
...
## What matters most in `gitforge`

These are the checks that catch real defects in this repository. Work through
them before the generic ones.

```
pkg/changelog/domain/entities/   Changelog parsing, dedup, insertion, section handling
pkg/config/                      Config + ProviderConfig, token resolution, loaders
pkg/git/                         AdapterFinder port + go-git GitOperations
pkg/global/, pkg/providers/, pkg/registry/, pkg/signing/
test/builders/, test/doubles/    builders and hand-written stubs
```

- **Every bounded context under `pkg/` owns its own `domain/` and `infrastructure/`.** A file placed outside that shape, or a domain package importing an SDK, is a Critical finding.
- **This is a library — exported symbols are contracts.** `autobump`, `autoupdate`, `code-guru`, and `dev-toolkit` compile against them, and several re-export the entities as type aliases. A signature change needs the three-place breaking-change flag and a MAJOR bump.
- **Changelog processing must round-trip.** `Process`, `ProcessNew`, `UpdateSection`, `MakeNewSections`, `ParseUnreleasedIntoSections`, `FixSectionHeadings`, and `DeduplicateEntries` operate on real users' `CHANGELOG.md` files. A change needs a test with a messy real-world input, including an empty `Unreleased` section and duplicate-but-differently-worded entries.
- **Token resolution must never leak.** `ProviderConfig.ResolveToken` expands environment variables and file paths; `ResolveTokenFromEnv`/`TokenEnvHint` derive provider-specific names. A token must never appear in an error message, a log line, or a remote URL that gets printed. **Critical.**
- **Signing is security-critical.** GPG and SSH signing paths must fail closed: if a signature cannot be produced, the commit must not be silently pushed unsigned.
- **Adding a provider means implementing the existing port and registering it in the factory registry** — not adding a `switch` on the service type. See the [Mapper Design Pattern](https://github.com/rios0rios0/guide/wiki/Mapper-Design-Pattern).
- Config loading accepts a file path or an HTTP(S) URL; a URL source must be validated and size-bounded before it is parsed.
...
- `t.Parallel()` at parent and subtest level, except where the test calls `t.Setenv` or `t.Chdir` (a Go restriction) or mutates global state such as `SetAdapterFinder` — those must not be parallel.
- Stubs for every key interface live in `test/doubles/` (`ForgeProviderStub`, `MirrorProviderStub`, …); builders live in `test/builders/`.
- `testify` for assertions — never bare `t.Error`/`t.Fatal`.
...
- The absence of a binary — this module is a library by design.
