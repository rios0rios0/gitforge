...
func TestReadSSHSigningKey(t *testing.T) {
	...
	t.Run("should return error when signing key is empty", func(t *testing.T) {
		...
		// when
		_, err := signing.ReadSSHSigningKey(keyPath)
...
func TestSignSSHCommit(t *testing.T) {
	t.Parallel()
	...
	t.Run("should return error when ssh-keygen is given invalid key", func(t *testing.T) {
		...
		// when
		_, err = signing.SignSSHCommit(context.Background(), content, fakeKey)

		// then
		require.Error(t, err)
		assert.Contains(t, err.Error(), "ssh-keygen signing failed")
	})
	...
	t.Run("should sign commit content with valid SSH key", func(t *testing.T) {
		...
		// when
		sig, err := signing.SignSSHCommit(context.Background(), content, keyPath)
