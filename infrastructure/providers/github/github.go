...
func (p *Provider) ListOpenPullRequests(
	...
	var allPRs []entities.PullRequestDetail
	...
	for {
		...
		for _, pr := range prs {
			allPRs = append(allPRs, entities.PullRequestDetail{
...
func (p *Provider) GetPullRequestFiles(
	...
	var allFiles []entities.PullRequestFile
	...
	for {
		...
		for _, f := range files {
			allFiles = append(allFiles, entities.PullRequestFile{
				Path:      f.GetFilename(),
