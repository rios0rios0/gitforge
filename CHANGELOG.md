...
- refreshed `.github/copilot-instructions.md` to reflect the Go `1.27.1` version
...
- pointed the pull request checklist at `make lint`, `make test` and `make sast` instead of `go test`, which skips the suites behind the `unit` build tag and runs neither the linter nor the SAST suite
...
- refreshed `.github/copilot-instructions.md` to reflect the Go `1.27.0` upgrade
